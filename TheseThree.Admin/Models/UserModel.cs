...
namespace TheseThree.Admin.Models
{
    public class UserModel
    {
            ...
            try
            {
                using (var dao = TheseThreeDao.GetInstance())
                {
                    ...
                    if (result != null && result.Rows.Count > 0)
                    {
                        ...
                        var userTypes = Convert.ToString(row["usertype"]);
                        if (userTypes.Contains(","))
                        {
                            user.UserTypes = userTypes.Split(',').ToList();
                        }
        ...
        public static Message GetEndUsers(int hospitalid,string name, string phone, string loginId, string deptname, string deptcode, string gwcode, string gwname, string zccode, string zcname,string lvcode,string lvname,string decode,string dename,string xzcode,string xzname)
        {
            var message = new Message
            {
                Status = MessageType.Fail,
        ...
        public static Message GetEndUsers(int examid,int hospitalid, string name, string loginId, string deptname, string deptcode, string gwcode, string gwname, string zccode, string zcname, string lvcode, string lvname, string xzcode, string xzname)
        {
            var message = new Message
            {
                Status = MessageType.Fail,
